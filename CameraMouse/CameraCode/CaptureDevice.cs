...
namespace CameraMouse
{
    ...
    public class CaptureDevice : IVideoSource
    {
        ...
        private Size videoSize = new Size(352, 288);
        ...
        public void WorkerThread()
        {
            ...
            try
            {
                ...
                if (sg.GetConnectedMediaType(mt) == 0)
                {
                    ...
                    videoSize = new Size(vih.BmiHeader.Width, vih.BmiHeader.Height);
            ...
            catch (Exception e)
            {
                try
                {
                    if (CameraMouseSuite.CMSLogger.CanCreateLogEvent(true, true, false, "CMSLogExceptionEvent"))
                    {
                        CameraMouseSuite.CMSLogExceptionEvent exceptionEvent
                            = new CameraMouseSuite.CMSLogExceptionEvent();
                     
                        exceptionEvent.SetException(e);
                        CameraMouseSuite.CMSLogger.SendLogEvent(exceptionEvent);
                    }
                }
                catch
                {
